...
9. **Serves everything through a FastAPI backend** and a single-page,
   dark-themed investigation dashboard (vis-network graph canvas + profile
   panels + candidate/influence feeds).
...
CSV / JSON data
 ...
 security.py            (RBAC + audit logging, wraps every  │
      │                   endpoint below)                   │
      ▼                                                    │
 api.py                 (FastAPI: /graph/data, /people/{id},│
                          /candidates, /influence, /link/…,  │
                          /audit-logs, /evaluation/summary)  │
      │
      ▼
 frontend/index.html    (vis-network dashboard, persona login,
...
├── frontend/
│   └── index.html                 # single-page dashboard (Tailwind + vis-network)
├── requirement.txt
└── README.md                      # this file
```
...
| Frontend | Static HTML/JS, Tailwind CSS (CDN), vis-network (CDN) |
...
| `GET /graph/data` | `graphs.read` | Progressive-disclosure graph payload for the canvas (ego-network around a focus node, or default top-influencer view) |
...
5 organizations, 10 locations. Full schema, noise ratios, and exactly how the
three planted "hidden links" and three planted "key influencers" map to the
raw data are documented in `Backend/data/README2.md`.
